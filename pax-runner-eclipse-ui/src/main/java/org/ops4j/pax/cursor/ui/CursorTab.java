...
/**
 * Pax Runner Eclipse Plugin Tab is an extension on org.eclipse.debug.ui.launchConfigurationTabs that adds extra options for Pax Runner
 * Eclipse Plugin. The extra options are lose coupled with Pax Runner Eclipse Plugin, as if this Cursor does not exist or is not installed
 * it will have no influence on Pax Runner Eclipse Plugin. The only connection between the two is through a configuration attribute that is
 * created by this tab on succesfull save (apply), atribute that contains a list of options that Pax Runner Eclipse Plugin will read and if
...
public class CursorTab extends AbstractLauncherTab {
	...
	/** true if run config was modified in this tab and the tab is still the active one */
	boolean tabModifiedAndActive = false;
	...
	/**
	 * Saves the configuration attributes by delegating to component blocks. First it will save a "null" Pax Runner Eclipse Plugin options
	 * so every block will have the chance to add it's own options.
	 * 
	 * @see ILaunchConfigurationTab#performApply(ILaunchConfigurationWorkingCopy)
	 */
	@SuppressWarnings("rawtypes")
	public void performApply(final ILaunchConfigurationWorkingCopy configuration) {
		// without this check changes from other tabs would be overwritten on saving
		if (tabModifiedAndActive) {
