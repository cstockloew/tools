...
public class ServiceManagment implements IServiceManagement{
	...
	public String getInstalledUnitsForService(String serviceId) {
		if(new File(Model.SERVICEFILENAME).exists()){
			...
			System.out.println("[ServiceManagement.getInstalledUnitsForService] the number of nodes for bundle: " + nodeList.getLength());
			for(int i = 0; i < nodeList.getLength(); i++){
				Element element = (Element) nodeList.item(i);
				services = services + "<unit><id>" + element.getAttribute("id") + 
						"</id><version>" + element.getAttribute("version") + "</version></unit>";
			}
	...
	public static Element getService(String serviceId, Document doc){
		...
		for(int i = 0; i < nodeList.getLength() ; i++){
			...
			System.out.println("[ServiceManagent.getService] has a node with serviceId: " + el.getAttribute("serviceId"));
			if(el.getAttribute("serviceId").equals(serviceId)){
				System.out.println("[ServiceManagement.getService] there is a service with id: " + serviceId);
				return el;
			}
		}
