...
public class ServiceRegistration implements IServiceRegistration {
	...
	/**
	 * TODO: register the installed bundles? Can use Map<bundleId, bundleVersion> as the second parameter
	 * Can call this (registerService)
	 */
	...
	/**
	 * Register app together with the installed bundles for a service. Use Map<bundleId, bundleVersion> as the second parameter
	 * Can call this (registerService)
	 */
/*	public boolean registerAppAndBundles(String serviceId, String appId, Map bundles) {
		return addAppAndBundlesToXML(serviceId, appId, bundles);
	}
	

	private boolean addAppAndBundlesToXML(String serviceId, String appId,
			Map bundles) {
		...
		try {
			...
			Iterator entries = bundles.entrySet().iterator();
			while (entries.hasNext()) {
			    Map.Entry entry = (Map.Entry) entries.next();
			    String bundleId = (String)entry.getKey();
			    String bundleVersion = (String)entry.getValue();
			    System.out.println("[ServiceRegistration.addAppAndBundlesToXML] bundleId = " + bundleId + ", bundleVersion = " + bundleVersion);
			    Element bundleEl = doc.createElement("bundle");
			    bundleEl.setAttribute("id", bundleId);
...
/*	public boolean registerService(String serviceId) {		
		return addServiceToXML(serviceId);
	}  */

/*	private boolean addServiceToXML(String serviceId) {
		// add the initial service element
		boolean success = true;
		Document doc = Model.getSrvDocument();
		Element appRoot = doc.createElement("service");
		doc.getDocumentElement().appendChild(appRoot);
		appRoot.setAttribute("serviceId", serviceId);
		 try {
			TransformerFactory.newInstance().newTransformer().transform(
			            new DOMSource(doc), new StreamResult(Model.SERVICEFILENAME));
		} catch (Exception e) {
	...
	private boolean addBundleToXML(String serviceId, String bundleId, String bundleVersion){
		boolean success = true;
		try {
			Document doc = Model.getSrvDocument();
...
/*	private boolean registered(String serviceId) {
		Document doc = Model.getSrvDocument();
		NodeList nodeList = doc.getChildNodes();
		for(int i = 0; i < nodeList.getLength() ; i++){
			Element el = (Element) nodeList.item(i);
			if(el.getAttribute("serviceId").equals(serviceId)){
				System.out.println("[ServiceRegistration.registered] has a service with id: " + serviceId);
				return true;
			}
		}
		System.out.println("[ServiceRegistration.registered] there is no service with id: " + serviceId);
		return false;
	}
