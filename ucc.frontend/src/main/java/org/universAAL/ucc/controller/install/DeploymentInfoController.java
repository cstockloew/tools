...
import org.universAAL.middleware.deploymaneger.uapp.model.DeploymentUnit; //import org.universAAL.middleware.interfaces.mpa.model.DeploymentUnit;
import org.universAAL.middleware.interfaces.PeerCard;
import org.universAAL.middleware.interfaces.PeerRole; //import org.universAAL.middleware.interfaces.mpa.model.Part;
...
public class DeploymentInfoController implements Button.ClickListener,
	...
	for (UAPPPart uapp : aal.getUaapList()) {
	    ...
	    DeployStrategyView dsv = new DeployStrategyView(aal.getName(), aal
		    .getServiceId(), uapp.getUappLocation());
	...
	if (event.getButton() == win.getOk()) {
		...
		if (uapp.getPart().getPartId().equals(selected)) {
		    ...
		    uapack = new UAPPPackage(aal.getServiceId(), uapp
			    .getAppId(), uf.toURI(), config);
		    InstallationResultsDetails res = installer
			    .requestToInstall(uapack);
		    // Testing
		    System.out
			    .println("[DeploymentInfoController] the installation results: "
				    + res.getGlobalResult().toString());
		    if (/*
			 * res.getGlobalResult().equals(InstallationResults.SUCCESS
			 * )
			 */true) {
			srvRegistration.registerApp(aal.getServiceId(), uapp
				.getAppId());
			// get bundles for each part in the appId;
			// for each bundle:
			srvRegistration.registerBundle(aal.getServiceId(), uapp
				.getBundleId(), uapp.getBundleVersion());
	...
	if (event.getProperty().getValue().toString().equals(
		bundle.getString("opt.selected.nodes"))) {
     ...
     */
    private Map<PeerCard, List<Part>> buildDefaultInstallationLayout(
	    UAPPPart uapp) {
	...
	for (UAPPPart ua : aal.getUaapList()) {
		...
		if (ua.getPart().getDeploymentUnit() == null) {
		    ...
		    System.out
			    .println("[DeployInfoController.buildDefaultInstallationLayout] add a part: "
				    + ua.getPart()
				    + " for peer: "
				    + peer.getPeerID());
		    if (plist == null)
		...
		if (checkDeployementUnit(ua.getPart().getDeploymentUnit(), peer)) {
		    System.out
			    .println("[DeployInfoController.buildDefaultInstallationLayout] IN CHECK DEPLOYMENT UNIT!");
		    List<Part> plist = mpaLayout.get(peer);
		    System.out
			    .println("[DeployInfoController.buildDefaultInstallationLayout] add a part: "
				    + ua.getPart()
	...
	if (dcvMap.get(selected).getSelect().getValue() != null
		&& !(dcvMap.get(selected).getSelect().getValue().toString()
			.equals(""))) {
	    String selPeer = dcvMap.get(selected).getSelect().getValue()
		    .toString();
		...
		List<Part> plist = mpaLayout.get(peer);
		System.out
			.println("[DeployInfoController.buildUserInstallationLayout] add a part: "
				+ uapp.getPart()
				+ " for peer: "
				+ peer.getPeerID());
		if (plist == null)
