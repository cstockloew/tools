...
//			}
			...
			RoomsWindow rWin = null;
			TabForm tab = (TabForm)tabSheet.getSelectedTab();
			for(Window w : app.getMainWindow().getChildWindows()) {
				if(w instanceof RoomsWindow) {
					rWin = (RoomsWindow)w;
					if(rWin.getUserTree().containsId(tab.getField("Device Address:").getValue())) {
						rWin.getUserTree().removeItem(tab.getField("Device Address:").getValue());
						if(rWin.getRwc().getTabSheet().getComponentCount() > 0)
