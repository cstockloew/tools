...
public class PreferencesController implements ClickListener {
	...
	public PreferencesController(UccUI app, PreferencesWindow win) {
		...
		ServiceReference ref = bc.getServiceReference(UserAccountDB.class.getName());
	...
	public void buttonClick(ClickEvent event) {
		if(event.getButton() == win.getSave()) {
			...
			if(win.getLangSelect().getValue().toString().equals("German") || win.getLangSelect().getValue().toString().equals("Deutsch")) {
				pref.setLanguage("de");
			} else {
				pref.setLanguage("en");
			}
