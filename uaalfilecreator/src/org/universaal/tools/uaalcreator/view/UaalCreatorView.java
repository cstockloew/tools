...
public class UaalCreatorView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		lab.setText("Please select the required bundles here:");
	    ...
	    addBundleBtn.setText("Add a required bundle");
	    addBundleBtn.addListener(SWT.Selection, new Listener() {
			@Override public void handleEvent(Event event) {
		        FileDialog fd = new FileDialog(Display.getCurrent().getActiveShell(), SWT.SAVE);
		        fd.setText("Add a required bundle");
	    ...
	    createUaalFileBtn.setText("Create the *.uaal file");
	    createUaalFileBtn.addListener(SWT.Selection, new Listener() {
			@Override public void handleEvent(Event event) {
				if (listBundles.getItemCount() <= 0) {
					...
					messageDialog.setMessage("Please specify the required bundles\n" +
