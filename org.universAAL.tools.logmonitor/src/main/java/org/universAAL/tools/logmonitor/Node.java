 ...
 * corresponds to a Node in the visualization and every property to an
...
public class Node {
	...
	/**
	 * Color for type node. Type nodes are nodes which have a property
	 * rdfs:type pointing at them.
	 */
	 ...
	 * @return
	 */
	private static Node copyFrom(Resource r,
			Hashtable<Resource, Node> visitedElements) {
		...
		if (node == null) {
			...
			Hashtable<String, Resource> temp
				...
				if (val instanceof Resource) {
					...
					while (iter.hasNext()) {
						...
						if (o instanceof Resource) {
							Node existingChild
									= visitedElements.get((Resource) o);
							if (existingChild == null) {
								child = null;
								...
								node.children.add(new Edge(key, existingChild,
										true));
							}
						} else {
							child = new Node("");
							Edge edge = new Edge(key + "(List " + i + ")",
									child);
							node.children.add(edge);
						}
						i++;
					...
					if (type == null)
						node.children.add(
								new Edge(key, new Node("<unknown>")));
					else
						node.children.add(
								new Edge(key, new Node(type, val.toString())));
				}
			}
	...
	Point drawNode(Graphics2D g, int xoff, int yoff) {
		if (x==-1) {
			...
			if (isLiteral) {
				...
				height = GraphPanel.fmNormal.getHeight()+border;
				if (theClass != null) {
					int width2 = GraphPanel.fmBold.stringWidth(theClass
							.getName())+border;
					if (width < width2)
						width = width2;
