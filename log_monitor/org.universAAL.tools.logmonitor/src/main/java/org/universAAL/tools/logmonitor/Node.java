...
public class Node {
	...
	if (node == null) {
		...
		node.theClass = ((UnmodifiableResource) r)
		...
		if (val instanceof Resource) {
			...
			if (o instanceof Resource) {
			    Node existingChild = visitedElements
				    .get((Resource) o);
			    if (existingChild == null) {
				child = null;
			...
			node.children.add(new Edge(key, new Node(type, val
				.toString())));
		}
	    }
	...
	if (x == -1) {
		...
		int width2 = GraphPanel.fmBold.stringWidth(literalType)
			+ border;
		if (width < width2)
		    width = width2;
	...
	if (isLiteral) {
	    ...
	    y += GraphPanel.fmBold.getDescent()
		    + GraphPanel.fmBold.getLeading();
