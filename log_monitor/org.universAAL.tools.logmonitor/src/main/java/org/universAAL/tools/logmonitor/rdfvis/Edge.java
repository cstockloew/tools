...
public class Edge implements Comparable<Object> {
	...
	if (parent == GraphPanel.selectedNode
		|| child == GraphPanel.selectedNode)
	...
	if (visited) {
	    int right = Math.max(currentWidth, x + textWidth)
		    + GraphPanel.nodeBorder;
	    g.drawLine(x - 15, y, right, y);
	    g.drawLine(right, y, right, child.y + child.height / 2);
	    drawArrow(g, right, child.y + child.height / 2, child.x
		    + child.width, child.y + child.height / 2);
	    g.setStroke(new BasicStroke(1F));
