...
public class RTFStatsExporter extends AbstractStatsExporter {
    ...
    protected void doGenerate(Stats stats, List details, File outputFile)
        ...
        try {
            ...
            pageHeaderAndFooterFont = new RtfFont(getMainFontName(), 9, Font.BOLDITALIC, new Color(
                    200, 200, 200));
            tableHeaderAndFooterFont = new RtfFont(getMainFontName(), getMainFontSize(), Font.BOLD);
            ...
            String intro = NLS.bind(Messages.MarkerStatsView_lblOverviewMessage, new Object[] {
            ...
            if (details.size() > 0) {
                try {
                    ...
                    String detailText = NLS.bind(Messages.MarkerStatsView_lblDetailMessage,
                    ...
                    doc.add(new Paragraph("An error occured while reading the selected markers",
                            mainFont));
                    CheckstyleLog.log(e, "An error occured while reading the selected markers");
                }
            }
    ...
    private void createDetailSection(List details, Document doc) throws CoreException,
        DocumentException {
        ...
        Cell resourceHeader = new Cell(new Chunk(Messages.MarkerStatsView_fileColumn,
        ...
        Cell folderHeader = new Cell(new Chunk(Messages.MarkerStatsView_folderColumn,
                tableHeaderAndFooterFont));
        folderHeader.setHorizontalAlignment(Element.ALIGN_CENTER);
        folderHeader.setHeader(true);
        table.addCell(folderHeader);
        Cell lineHeader = new Cell(new Chunk(Messages.MarkerStatsView_lineColumn,
                tableHeaderAndFooterFont));
        lineHeader.setHorizontalAlignment(Element.ALIGN_CENTER);
        lineHeader.setHeader(true);
        table.addCell(lineHeader);
        Cell messageHeader = new Cell(new Chunk(Messages.MarkerStatsView_messageColumn,
                tableHeaderAndFooterFont));
        ...
        for (Iterator iter = details.iterator(); iter.hasNext();) {
            ...
            fileCell = new Cell(new Chunk(
                    marker.getResource().getParent().getFullPath().toString(), mainFont));
            table.addCell(fileCell);
            lineCell = new Cell(new Chunk(marker.getAttribute(IMarker.LINE_NUMBER).toString(),
                    mainFont));
            lineCell.setHorizontalAlignment(Element.ALIGN_CENTER);
            table.addCell(lineCell);
            messageCell = new Cell(new Chunk(marker.getAttribute(IMarker.MESSAGE).toString(),
                    mainFont));
    ...
    private void createSummaryTable(Stats stats, Document doc) throws DocumentException {
        ...
        Cell typeHeader = new Cell(new Chunk(Messages.MarkerStatsView_kindOfErrorColumn,
                tableHeaderAndFooterFont));
        typeHeader.setHorizontalAlignment(Element.ALIGN_CENTER);
        typeHeader.setHeader(true);
        table.addCell(typeHeader);
        Cell countHeader = new Cell(new Chunk(Messages.MarkerStatsView_numberOfErrorsColumn,
                tableHeaderAndFooterFont));
     ...
     * file.
     * 
     * @param outputFile the output file as given by the user
